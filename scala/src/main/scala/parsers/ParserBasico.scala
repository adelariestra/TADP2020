...
def anyChar(cadena :String) :ResultadoParseo[Char] ={
  ResultadoParseo(cadena).map(elemento => (elemento.head, elemento.tail))
}
}
...
override def getResultado(cadena: String): ResultadoParseo[Char] = {
  ResultadoParseo(cadena).map(elemento => ResultadoParseo(elemento.head, elemento.tail))
}
}
