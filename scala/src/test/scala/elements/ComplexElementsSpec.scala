...
class ComplexElementsSpec extends AnyFlatSpec with should.Matchers {
  it should "parsear correctamente un grupo" in {
    group("grupo(\n   \t triangulo[200 @ 50, 101 @ 335, 299 @ 335],\n   \t circulo[200 @ 350, 100]\n    )").get shouldEqual (ResultadoParseo(GroupFigure(List(TriangleFigure((200, 50), (101, 335), (299, 335)), CircleFigure((200, 350), 100))), ""))
  }
  ...
  it should "parsear correctamente una escala con un componente" in {
    escala("escala[6, 15](\n\tcirculo[0 @ 5, 10]\n)").get shouldEqual (ResultadoParseo(EscalaTr((CircleFigure((0, 5), 10)), 6,15), ""))
  }

  it should "parsear correctamente una rotacion con un componente" in {
    rotacion("rotacion[6](\n\tcirculo[0 @ 5, 10]\n)").get shouldEqual (ResultadoParseo(RotacionTr((CircleFigure((0, 5), 10)),6), ""))
  }

  it should "parsear correctamente una traslación con un componente" in {
    traslacion("traslacion[6, 15](\n\tcirculo[0 @ 5, 10]\n)").get shouldEqual (ResultadoParseo(TraslacionTr((CircleFigure((0, 5), 10)),6,15), ""))
  }

  it should "parsear correctamente una color con un componente" in {
    color("color[6, 6, 6](\n\tcirculo[0 @ 5, 10]\n)").get shouldEqual (ResultadoParseo(ColorTr((CircleFigure((0, 5), 10)),6,6,6), ""))
  }
